/* (LOAD macros.ps) */
var __arrayRemove = function (thing) {
    var i = 0;
        ...
        if (!alreadyChild) {
            ...
            if (name) {
                return parent.once('change' + ':' + name, function (e) {
                    return __arrayRemove(child._parents, e.target);
                });
            } else {
                return parent.on('remove', function (e) {
                    return e.target === child ? __arrayRemove(child._parents, e.target) : null;
                });
            };
        };
    };
...
Model.prototype.remove = function (obj, silent) {
    var retval = __arrayRemove(this._storage, obj);
    if (retval) {
        --this.length;
        if (Modelp(obj)) {
            __arrayRemove(obj._parents, this);
        };
        if (!silent) {
...
Model.prototype.clear = function (silent) {
    ...
    for (var thing = null, _js_idx8 = 0; _js_idx8 < oldStorage.length; _js_idx8 += 1) {
        ...
        if (Modelp(thing)) {
            __arrayRemove(thing._parents, this);
        };
    };
    if (!silent) {
...
function View(options) {
    var fun = function (model) {
        ...
        if (options) {
            ...
            var classes = (options.className && options.type ? (options.className.indexOf(' ') === 0 ? options.type + options.className : options.className) : (options.className ? options.className : options.type)).split(" ");
            for(var c in classes){
                console.log(classes[c]);
                this.$el.addClass(classes[c]);
            }
            if (!options.render) {
