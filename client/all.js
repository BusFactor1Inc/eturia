...
Model.prototype.size = function(dims) {
    var size = 1;
    var pos = {};
    for(var dim in dims) {
        size = size * dims[dim];
...
Model.prototype.peek = function(pos) {
    pos = pos || this.pos;
    var offset = 0;
    for(var i in this.dims) {
        offset += pos[i]*this.dims[i];
    }
    this.pos = pos;
    return this.current(this.at(offset));
};
Model.prototype.poke = function(thing, pos) {
    pos = pos || this.pos;
    var offset = 0;
    for(var i in this.dims) {
        offset += pos[i]*this.dims[i];
    }
    this.remove(offset, true);
    this.insertAt(offset, thing, true);
    this.pos = pos;
    this.current(offset);
    return thing;
};
...
/* TODO: decide if want to remove */
    ...
    saveCore: function (path) {
        ...
        localStorage.setItem(JSON.stringify(code));
        return true;
    },
